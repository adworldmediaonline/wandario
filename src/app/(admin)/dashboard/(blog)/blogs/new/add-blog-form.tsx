...
import { addBlogAction } from '@/app/actions/blog-action';
...
export default function AddBlogForm({ categories }: AddBlogFormProps) {
  ...
  const { execute, status } = useAction(addBlogAction, {
    onSuccess(data) {
      if (data?.data?.success) {
        toast('Blog add successful', {
          closeButton: true,
          description: 'Blog add successful',
        });
        router.push('/dashboard/blogs');
      }
      if (!data?.data?.success) {
        toast('Blog add failed', {
          description: data?.data?.message ?? 'Blog add failed',
        });
      }
    },
    onError(error) {
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabelInfo
                            label="Category"
                            required
                            tooltip="Select the category this blog belongs to"
                          />
                          <Select
