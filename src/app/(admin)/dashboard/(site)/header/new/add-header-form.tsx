...
import { addHeaderAction } from '@/app/actions/header-action';
...
export default function AddHeaderForm({ pages }: AddHeaderFormProps) {
  ...
  const { execute, status } = useAction(addHeaderAction, {
    onSuccess(data) {
    ...
    onError({ error }) {
      toast.error('Error creating header', {
